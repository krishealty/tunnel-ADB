...
opt_install () {
        ...
        if [ $? == 0 ]
                ...
                then
                        ...
                        clear
                        chmod +x tunnel-ADB.sh
                        touch ~/.bash_aliases 2> /dev/null
                        mkdir $PWD/.temp 2> /dev/null
                        echo "alias adb-toolkit='cd $PWD && sudo bash tunnel-ADB.sh'" >> ~/.bash_aliases
                        source ~/.bash_aliases
                        echo -e "\n\e[1;92mINSTALLATION COMPLETED\n"
                        echo -e "\e[1;92mUSAGE = \e[1;91m'sudo ./tunnel-ADB.sh' or you can do 'sudo adb-tookit' from anywhere in shell\e[0m\n\n"
